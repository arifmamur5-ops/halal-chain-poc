...
contract HalalChainRegistry is AccessControl {
    ...
    event CertificationApproved(
        uint256 indexed productId, address indexed approver, uint256 currentApprovals, uint256 required
    );
        ...
        onlyRole(CERTIFIER_ROLE)
    {
        ...
        certificates[productId] =
            Certificate(msg.sender, ipfsHash, block.timestamp, block.timestamp + validDays * 1 days, false);
    ...
    function revokeCertificate(uint256 productId, string calldata reason) external onlyRole(CERTIFIER_ROLE) {
    ...
    function getProductFullHistory(uint256 productId)
        external
        view
        returns (Product memory product, Certificate memory certificate, address[] memory custody, bool certValid)
    {
